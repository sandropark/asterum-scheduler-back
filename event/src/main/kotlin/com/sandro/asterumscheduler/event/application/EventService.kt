...
class EventService(
    ...
    fun create(request: CreateEventRequest): EventResponse {
        ...
        event.replaceParticipants(request.participantIds)
    ...
    @Transactional
    fun update(id: Long, request: UpdateEventRequest): EventResponse {
        if (request.locationId != null && !locationReader.existsById(request.locationId))
            throw BusinessException(ErrorCode.NOT_FOUND)
        if (request.participantIds.isNotEmpty() && userReader.findNotExistingIds(request.participantIds).isNotEmpty())
            throw BusinessException(ErrorCode.NOT_FOUND)
        ...
        try {
            return eventRepository.saveAndFlush(event).toResponse()
        } catch (e: DataIntegrityViolationException) {
            throw BusinessException(ErrorCode.CONFLICT, e)
        }
    }
